...
export default function LogEntryModal({ item, onClose, onSave, onDelete }) {
  ...
  function toggleGenre(nextGenre) {
    const current = genre
      .split(",")
      .map((value) => value.trim())
      .filter(Boolean);
    const exists = current.some(
      (value) => value.toLowerCase() === nextGenre.toLowerCase(),
    );
    setGenre(
      (exists
        ? current.filter(
      ...
      >
        ...
        <div className="log-modal__body">
          ...
          <div className="log-modal__main">
            ...
            <label className="log-modal__genre-field">
              <span className="log-modal__field-label">Genres</span>
              <input
                value={genre}
            ...
            <div className="log-modal__genre-chips">
              {QUICK_GENRES.map((quickGenre) => (
                <button
                  key={quickGenre}
                  type="button"
                  className={`genre-chip ${
                    activeGenres.includes(quickGenre.toLowerCase())
                      ? "genre-chip--active"
                      : ""
                  }`}
                  onClick={() => toggleGenre(quickGenre)}
                >
                  {quickGenre}
                </button>
              ))}
